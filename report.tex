...
Confounding is the most severe failure mode. With even 1 latent node, TsP degrades from SHD $0.54$ to $3.72$ ($7\times$ worse), while Random is nearly unaffected ($1.96 \to 2.16$). Replacing graph-coloring intervention groups with strongly separating sets~\cite{kocaoglu2017experimental} (TsP-Sep) yields a modest but consistent improvement across all confounded settings (e.g., SHD $3.22$ vs.\ $3.72$ at 1 latent), yet both TsP variants remain substantially worse than Random for all $\geq 1$ latents. At 4 latents the advantage of TsP-Sep reverses slightly ($5.00$ vs.\ $4.96$), suggesting separating sets alone do not resolve the fundamental failure.

We further tested two heuristic corrections to the allocation policy: (1) a quadratic KL penalty that down-weights groups where no candidate fits well ($\alpha_k \propto 1/\text{min-KL}_k^2$), and (2) a stagnation-based fallback that reverts to uniform random allocation when the KL landscape has not improved over a sliding window. Both modifications leave SHD unchanged across all confounding levels. This reveals that the failure is \emph{not} in the allocation policy itself — it is in the \emph{graph estimate}: $d^*_k = \argmin_d \text{KL}(\hat{P} \| P_d)$ selects the wrong MPDAG regardless of how samples are distributed, because every candidate MPDAG is systematically mis-specified under confounding. Fixing allocation without fixing the candidate model space cannot recover the true graph.}
